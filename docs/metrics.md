...
| `sage_relay_latency_seconds` | histogram | `service_id` | Upstream relay attempt latency in seconds — selection through response, one observation per attempt. Not client-facing latency: a request that retried or hedged is several observations, none of them its total. |
...
| `sage_relay_total` | counter | `service_id`, `status` | Upstream relay attempts by service and HTTP status: one count per attempt inside retry, hedge and batch, so a retried or hedged request counts more than once. Cache hits and coalesced requests make no attempt and are absent. One count per client request is sage_client_requests_total. |
