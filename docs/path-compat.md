...
| Log level at runtime | n/a | `SAGE_LOG_LEVEL` overrides the file at startup (announced as a warning); `GET`/`PUT /admin/log-level` moves it live per process, not persisted — 2026-09-13 | The canary's config is a sealed secret at `error`; a look at a live problem meant editing the secret and restarting |
...
| `external_block_sources` at runtime | n/a | `GET`/`PUT`/`DELETE /admin/external-sources/{service}` since 2026-09-13: replace or stop a service's sources on the running process, with each source's last poll status; per process, not persisted, the file's sources return on restart | Thirteen of the canary's sixty-eight sources were retired, misnamed or rate-limited and the file is a sealed secret; a dead source polls every fifteen seconds until someone can edit it |
