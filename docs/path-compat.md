...
| Gateway-generated errors (no usable supplier response, timeout) | HTTP 500, JSON-RPC code `-31001` with `data.retryable` | HTTP 500 (504 on timeout, 429 on rate limit, 400 on a client mistake, 499 since 2026-09-13 when the client hung up before an answer, 502 since 2026-09-15 when a supplier's response exceeds `max_response_body_bytes`), JSON-RPC code `-32603`, no `data`. Was HTTP 200 until 2026-09-04 | The status is what a load balancer, a dashboard and a client's retry policy branch on; a 200 hid every failure from all three. `-32603` is the standard code for it; PATH's `-31001` is its own invention and a client keyed on it is a client keyed on PATH |
...
| Reputation key for an operator staking one host per RPC type | Keyed per endpoint/domain from the address | Per-URL keys, method-block hosts and the admin listing name the host the face is actually dialed from, since 2026-09-13 (`protocol.URLResolver`); kleomedes' and highstakes' REST faces on osmosis were scored and listed under their JSON-RPC hosts, both faces on one score | A face is served by a host; the score should be that host's |
