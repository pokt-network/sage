...
| `rpc_type_fallbacks` | Per service, a request whose RPC type the supplier has not staked is relayed through the URL of the mapped type (`comet_bft: json_rpc`) | Honoured since 2026-08-31: same key, same one-hop semantics, validated at load | Mainnet cosmos suppliers are still catching up on their `comet_bft` stakes; without the mapping they are invisible to `comet_bft` traffic and the cosmos health check probes them with a type they cannot take, grading healthy suppliers `minor_error` every cycle |
